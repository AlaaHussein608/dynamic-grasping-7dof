...
=========================
Benchmarks the Adaptive Sliding Mode Controller (SMC) on the Franka Panda
using both Quintic and TOPPRA trajectory planners.
...
def run_smc(traj_type, q_target, viewer,
            disturbance_tau=None, sensor_noise_std=0.0):
    """Run one SMC episode to q_target and return raw history arrays."""
    reset()
    ...
    while True:
        ...
        tau, _ = sliding_mode_control(q, qd, q_d, qd_d, qdd_d,
        ...
        if disturbance_tau is not None and step_idx < len(disturbance_tau):
            tau = tau + disturbance_tau[step_idx]
...
def main():
    ...
    for traj_type in ("toppra", "quintic"):
        ...
        for seed_idx, q_target in enumerate(TARGETS):
            ...
            r = run_smc(traj_type, q_target, viewer)
            metrics_normal.append(gather_metrics(r, q_target))

            r = run_smc(traj_type, q_target, viewer, disturbance_tau=disturbance)
            metrics_dist.append(gather_metrics(r, q_target))

            r = run_smc(traj_type, q_target, viewer, sensor_noise_std=SENSOR_NOISE_STD)
            metrics_noise.append(gather_metrics(r, q_target))

        print(f"  [{traj_type.upper()}] all {N_SEEDS} seeds done.              ")
        print_results("SMC", traj_type, N_SEEDS,
